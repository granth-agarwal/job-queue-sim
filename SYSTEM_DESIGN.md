...
`Job` is the state-bearing abstract base. It encapsulates a final job ID and creation timestamp, implements common logging, and requires each subtype to define `execute()`. `EmailJob`, `ReportGenerationJob`, and `DataSyncJob` provide distinct execution behavior, while `RetryableJob` adds stateful retry behavior.
...
`JobBuilder` validates the required ID and enum type, then selects a subtype with an exhaustive switch over `JobType`. `Worker.processJob()` invokes the subtype through the `Job` abstraction, catches checked execution failures, and records completion in `finally`.
...
| Strings | The standalone appendix demonstrates runtime-dependent pool/reference comparisons; StringBuilder and concurrent StringBuffer paths run successfully. |
...
- **Compile-time configuration:** Supported job types are fixed enum values and the retry ceiling is a constant; a production implementation would load operational retry policy from validated service configuration while keeping job types strongly typed.
...
- **Shallow builder validation:** `JobBuilder` checks ID presence and requires an enum type but ignores ID format and uniqueness; a production implementation would enforce a validation policy and reject duplicate idempotency keys at the persistence boundary.
...
- **Covered — unknown or invalid job type at build time:** `JobBuilder.setType()` accepts only `JobType`, so arbitrary strings do not compile, and `build()` rejects a null enum value.
...
- **Not covered — null Strings outside the builder path:** A null builder ID is rejected, but direct job constructors accept null; behavior then varies by subtype, including `null@example.test` for email and a possible `NullPointerException` in report generation, so constructor-level validation is needed.
...
1. Introduce a validated job envelope with attempt metadata and an idempotency key while preserving immutable job identity.
