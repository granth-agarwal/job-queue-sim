...
public final class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            for (int attempt = 1;
                 attempt <= RetryableJob.MAX_RETRY_ATTEMPTS + 1;
                 attempt++) {

                retryableJob.retry();

                System.out.println(
                        "[Retry] Recorded retry " + retryableJob.getRetryCount()
                );
            ...
            System.out.println(
                    "[Checked Exception] Handled retry limit: "
        ...
        try {
            ...
            System.out.println(
                    "[Unchecked Exception] Handled invalid config: "
                            + exception.getMessage()
            );
        }

        System.out.println("[StringBuilder] Processing two more jobs sequentially:");

        worker.processJob(new EmailJob("email-extra"));
        worker.processJob(new ReportGenerationJob("report-extra"));

        System.out.println(
                "[StringBuilder Summary] "
                        + LogFormatter.buildSummary(
                        List.of("email-1", "report-1", "sync-1", "retry-1")
                )
        );

        System.out.println(
                "[Static Counter] Total attempts processed: "
                        + Worker.totalJobsProcessed
        );

        System.out.println(
                "[Shared StringBuilder] Full worker log:\n"
                        + Worker.sharedLog
        );
    }
}